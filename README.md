...
在 Codex 打开本项目目录，发送这一句话：

> 阅读 SKILL.md，帮我连接并初始化一台闲置 VPS，保留当前 SSH 登录方式，完成后自动检查结果。

Codex 会引导你提供连接信息、检查已有服务、上传脚本、执行初始化并验收。无需先手动安装 Skill；自动操作需要可用的 SSH 登录方式和运行权限。详见 [Codex 使用指南](CODEX-GUIDE.md)。下面的三步教程适合手动操作。
